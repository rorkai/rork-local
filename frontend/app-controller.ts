export function setupAppController() {
  ...
  function setProgress(state, title, desc, pct?: number) {
  ...
  async function captureShot() {
    flashShutter();
    try {
      await postJSON("/api/screenshots/capture", {
  ...
  function applyJobStatus(job) {
    ...
    if (job.kind === "app-create" && !jobRunning && job.state !== "idle") {
      ...
      if (job.state === "success" && job.result?.appId) {
        ...
        cError.textContent =
          "App creation failed — see output above (a cached web session from `asc web auth login` is required)";
        show(cError);
      }
    }
